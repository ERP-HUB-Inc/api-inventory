import { EntityRepository, EntityManager, getConnection } from 'typeorm';
...
export default class MovementLogRepository {
  async findMany(productId: string, clientId: string): Promise {
    const results: MovementLog[] = await getConnection()
      .createQueryBuilder(MovementLog, 'ML')
      ...
      .where('productId = :productId AND ML.clientId = :clientId', {
        productId,
        clientId,
      })
