...
@Service()
export default class ApproveTransferService extends BaseService<StockTransfer> {
    ...
    constructor() {
        ...
        this.stockTransferEntryRepository = getCustomRepository(StockTransferEntryRepository);
    ...
    private async saveRecord(data: StockTransfer, condition?: object) {
        await getManager().transaction(async manager => {
            ...
            if (data.locationId == data.fromLocationId) {
                throw new NotFoundError("You was created this transfer, so we don't allow to receive", HttpCode.INVALID_LOCATION_FOR_RECEIVE);
            }
            ...
            if (condition != null && condition["id"] != null) {
                ...
                if (stockTransferData == null || stockTransferData.step != TransferStepEnums.PROCESS) {
                    throw new NotFoundError("Transfer is not found", HttpCode.TRANSFER_NOT_FOUND);
                }
            }

            stockTransferData.receiveDate = moment(moment().format("YYYY-MM-DD h:mm:ss A")).toDate();
            stockTransferData.step = TransferStepEnums.RECEIVED;
            stockTransferData.receiverId = data.userId;
            const resultStockTransfer = await manager.save(this.repository.entityName, stockTransferData);
            ...
            if (Array.isArray(entries)) {
                ...
                for (var i = 0; i < lenght; i++) {
                    ...
                    let entry = await this.stockTransferEntryRepository.findOneByFieldName({ id: entries[i].id });
                    entry.receiveQuantity = entries[i].receiveQuantity <= entry.transferQuantity ? entries[i].receiveQuantity * unitMultiple : entries[i].transferQuantity * unitMultiple;
                    const resultStockTransferEntry = await manager.save(this.stockTransferEntryRepository.entityName, entry);
