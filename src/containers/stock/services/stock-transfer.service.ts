...
@Service()
export default class StockTransferService extends BaseService<StockTransfer> {
    ...
    private async saveRecord(data: StockTransfer, condition?: object) {
        ...
        await getManager().transaction(async manager => {
            ...
            if (condition == null || _.isEmpty(condition["id"])) {
                ...
                stock.number = await Container.get(ClientAutoNumberService).getStockTransferNumber(manager, data.clientId);
                ...
                await manager.update(StockTransfer, {id: condition["id"], clientId: data.clientId}, stock);
            ...
            if (Array.isArray(entries)) {
                const promises = entries.map(async (stockTransferEntry) => {
                    ...
                    const sourceProductLocation = await this.repository.connection.createQueryBuilder(ProductVariant, "PV")
                    .leftJoin("PV.productLocations", "PL", "PL.locationId = :locationId", {locationId: stock.fromLocationId})
                    .select([
                    ...
                    .where("PV.id = :productVariantId AND PV.clientId = :clientId", {productVariantId, clientId: data.clientId})
                    .getRawOne();
                    ...
                    if (_.isEmpty(stockTransferEntry.id)) {
                        ...
                        entry.receiveQuantity = stock.step == TransferStepEnums.RECEIVED ? transferQuantity : 0;
                        entry.price = sourceProductLocation["cost"];
                        entry.status = _.isEmpty(stockTransferEntry.status) ? ModelStatus.ACTIVE : stockTransferEntry.status;
                        await manager.insert(StockTransferEntry, entry);
                    } else {
                        entry.transferQuantity = transferQuantity;
                        entry.price = stockTransferEntry.cost;
                        entry.status = stockTransferEntry.status;
                        await manager.update(StockTransferEntry, {stockTransferId, id: stockTransferEntry.id}, entry);
                    }
                    
                    if (transferStep == TransferStepEnums.RECEIVED) {
                        const receiveProductLocation: ProductLocation = await this.repository.connection.createQueryBuilder(ProductLocation, "PL")
                        .select([
                            "productVariantId",
                            "quantity"
                        ])
                        .where("PL.locationId = :locationId AND PL.productVariantId = :productVariantId", {locationId: stock.toLocationId, productVariantId})
                        .getRawOne();

                        let receiveQuantity: number = transferQuantity;

                        if (receiveProductLocation) {
                            const newQuantity: number = receiveQuantity + receiveProductLocation.quantity;
                            await manager.update(ProductLocation, {productVariantId, locationId: stock.toLocationId}, {quantity: newQuantity});
                        } else {
                        ...
                        await manager.update(ProductLocation, {productVariantId, locationId: stock.fromLocationId}, {quantity: leftQuantity});
                        ...
                        movementLog.oldQuantity = sourceProductLocation ? sourceProductLocation.quantity : 0;
                        ...
                        movementLog.oldQuantity = receiveProductLocation ? receiveProductLocation.quantity : 0;
                        await this.movementLogRepository.log(movementLog, StockEnums.TRANSFER, manager);
                    }
    ...
    async cancelStockTransfer(condition?: object) {
        ...
        const stockTransfer = await this.repository.findOneByFieldName(condition, ["step"]);
        if (stockTransfer == null) {
            throw new NotFoundError("Transfer is not found", HttpCode.TRANSFER_NOT_FOUND);
        }

        if (stockTransfer.step == TransferStepEnums.RECEIVED) {
            throw new BadRequestError("This transfer has been received", HttpCode.FORBIDEN_STEP_PROCESS);
        }
    ...
    async findManyOfReceiveList(filter: Filter) {
        ...
        const results = await this.stockTransferRepository.findManyOfReceiveList(filter);
