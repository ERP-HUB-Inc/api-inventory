...
export default class StockService {
    ...
    constructor() {
        ...
        this.productPackageRepository = getCustomRepository(ProductPackageRepository);
    ...
    public async transferProductStock(
        ...
        // PRODUCT VARAINT
        let productVariant = await this.productVariantRepository.findOneByFieldName({id: productVariantId}, ["id"]);
        if (productVariant == null) {
            throw new NotFoundError("Product variant not found", HttpCode.PRODUCT_NOT_FOUND);
        }
        ...
        let fromProductLocation = await this.productLocationRepository.findOneByFieldName({locationId: fromLocationId, productVariantId});
        if (fromProductLocation == null) { // This case we need to think about stock that allow negative quantity(mean that we allow you to transfer product in case you have not quantity)
            ...
            await manager.save(this.productLocationRepository.entityName, fromProductLocation);
        } else {

            if (quantity > fromProductLocation.quantity) {
                throw new BadRequestError("Not enought quantity for receive from source location", HttpCode.RECEIVE_STOCK_TRANSFER_NOT_ENOUGHT_QUANTITY);
            }
        ...
        // PRODUCT TO LOCATION
        let toProductLocation = await this.productLocationRepository.findOneByFieldName({locationId: toLocationId, productVariantId: productVariant.id});
        if (toProductLocation == null) {
            ...
            await manager.save(this.productLocationRepository.entityName, toProductLocation);
        } else {
            toProductLocation.quantity += quantity;
            await manager.save(this.productLocationRepository.entityName, toProductLocation);
        }
    ...
    public async adjustmentProductStock(
        ...
        let productLocation = await this.productLocationRepository.findOneByFieldName({locationId, productVariantId});
        if (productLocation == null) {
            ...
            await manager.update(ProductLocation, {productVariantId, locationId}, {quantity});
        }
    ...
    private async processProductStockOneByOne(
        ...
        console.log(`=================START PROCESS STOCK ONE BY ONE WITH QTY: ${quantity} INTO LOCATION ${locationId}=====================`);
        ...
        if (refType == StockEnums.PO) {
            if (beginingCost == 0 || (beginingQuantity + quantity) == 0)
                newCost = cost;
            else
                newCost = ((beginingCost * Math.abs(Math.max(beginingQuantity, 0))) + (cost * quantity)) / (Math.max(beginingQuantity, 0) + quantity);
        ...
        await manager.update(ProductVariant, {id: productVariant.id, clientId}, updateProductVariant);

        let productLocation: ProductLocation = await this.productLocationRepository.connection.createQueryBuilder<ProductLocation>("ProductLocation", "PL")
        .select([
            "id",
            "clientId",
            "locationId",
        ...
        .where("locationId = :locationId AND productVariantId = :productVariantId", {locationId, productVariantId: productVariant.id})
        .getRawOne();
        ...
        if (productLocation == null) {
            ...
            productLocation.clientId = clientId;
            await manager.update(ProductLocation, {id: productLocation.id, locationId, productVariantId: productVariant.id}, {quantity: productLocation.quantity, clientId});
        }
    ...
    public async returnProductStock(
        ...
        console.log("=================START RETURN PRODUCT STOCK=====================");

        this.productLocationRepository.clientId = clientId;
        this.itemRepository.clientId = clientId;
        await manager.update(ProductVariant, {id: productVariant.id, clientId}, {quantity: productVariant.quantity + quantity});

        // PRODUCT LOCATION
        let productLocation = await this.productLocationRepository.findOneByFieldName({locationId, productVariantId: productVariant.id});
        if (productLocation == null) {
            productLocation = new ProductLocation();
            ...
            await manager.update(ProductLocation, {clientId, productVariantId: productVariant.id, locationId}, {quantity: productLocation.quantity + quantity});
        }

    }

    public async makeFinalProductStock(manager: EntityManager, 
        productId: string,
        locationId: string,
        quantity: number,
        userId: string,
        ...
        console.log("=================START MAKE FINAL PRODUCT STOCK=====================");
        this.itemRepository.clientId = this.clientId;
        this.productPackageRepository.clientId = this.clientId;
        let product = await this.itemRepository.findOneByFieldName({id: productId });
        if (product == null) {
            throw new NotFoundError("Product is not found", HttpCode.PRODUCT_NOT_FOUND);
        }

        if(product.productOption == ProductionOptionEnums.COMPOSITE) {

            // Add Stock to Final Products
            let finalProductVariant = await this.productVariantRepository.findOneByFieldName({productId}); 
            await this.processProductStock(manager, finalProductVariant, locationId, quantity, userId, cost, StockEnums.MAKE, refId, refNo);
        
            // Cut Stock from Raw Products
            const list = await this.productPackageRepository.findMany({condition: {productId}});
            if(list != null) {
                const packageList = list[0];
                const length = packageList.length;
                for(var i = 0; i< length; i++) {
                    let packageProduct = packageList[i];
                    let rawProduct = await this.itemRepository.findOneByFieldName({id: packageProduct.rawProductId }); 
                    let rawProductVariant = await this.productVariantRepository.findOneByFieldName({productId: rawProduct.id }); 
                    await this.processProductStockOneByOne(manager, rawProductVariant, locationId, (-1) * quantity * packageProduct.quantity, 
                        userId, rawProductVariant.cost, StockEnums.MAKE, refId, refNo);
                }
            }

        } else {
            throw new BadRequestError("This product is not composite, so cannot do make production.", HttpCode.FORBIDEN_STEP_PROCESS);
        }
