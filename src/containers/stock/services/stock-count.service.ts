...
@Service()
export default class StockCountService extends BaseService {
  ...
  private async getUncountedProducts(
    clientId: string,
    locationId: string,
    exceptProductVariantIds: string[],
  ) {
    const query = this.repository.connection
      ...
      .leftJoin(
        'PV.productLocations',
        'PL',
        'PL.clientId = :clientId AND PL.locationId = :locationId',
        { locationId },
      )
      .select([
      ...
      .where('P.clientId = :clientId AND PV.clientId = :clientId', {
        clientId,
      });

    if (exceptProductVariantIds.length)
      query.andWhere('PV.id NOT IN(:productVariantIds)', {
        productVariantIds: exceptProductVariantIds,
      });
  ...
  async getStockCountById(user: User, stockCountId: string) {
    const stockCount: StockCount = await this.repository.connection
      .createQueryBuilder(StockCount, 'SC')
      ...
      .where('SC.clientId = :clientId AND SC.id = :stockCountId', {
        clientId: user.clientId,
        stockCountId,
      })
      .getOne();

    stockCount.stockCountEntries = await this.repository.connection
      .createQueryBuilder(StockCountEntry, 'SCE')
      ...
      .where('SCE.clientId = :clientId AND SCE.stockCountId = :stockCountId', {
        clientId: user.clientId,
    ...
    if (stockCount.type == 'PARTIAL') {
      const countedEntries = stockCount.stockCountEntries.filter(
        (stockCountEntry) => stockCountEntry.status == 'COUNTED',
      );
      stockCount.all = stockCount.stockCountEntries.length;
      stockCount.uncounted = stockCount.stockCountEntries.filter(
        (stockCountEntry) => stockCountEntry.status == 'UNCOUNTED',
      ).length;
      stockCount.counted = countedEntries.length;
      stockCount.unmatched = countedEntries.filter(
        (stockCountEntry) => stockCountEntry.expected != stockCountEntry.count,
      ).length;
      stockCount.matched = countedEntries.filter(
        (stockCountEntry) => stockCountEntry.expected == stockCountEntry.count,
      ).length;
    } else {
      const uncountedEntries = await this.getUncountedProducts(
        user.clientId,
        stockCount.locationId,
        stockCount.stockCountEntries.map(
          (stockCountEntry) => stockCountEntry.productVariantId,
        ),
      );
      stockCount.uncounted = uncountedEntries.length;
      stockCount.counted = stockCount.stockCountEntries.length;
      stockCount.unmatched = stockCount.stockCountEntries.filter(
        (stockCountEntry) => stockCountEntry.expected != stockCountEntry.count,
      ).length;
      stockCount.matched = stockCount.stockCountEntries.filter(
        (stockCountEntry) => stockCountEntry.expected == stockCountEntry.count,
      ).length;

      stockCount.stockCountEntries = [
        ...stockCount.stockCountEntries,
        ...uncountedEntries,
      ];
  ...
  async getStockCounts(filter: Filter) {
    ...
    const stockCounts: StockCount[] = await query
  ...
  async updateStockCountById(user: User, stockCount: StockCount, id: string) {
    await getManager().transaction(async (manager) => {
      await manager.update(
        ...
        {
          startDate: stockCount.startDate,
          startTime: stockCount.startTime,
          status: stockCount.status,
        },
      );

      const promises = stockCount.stockCountEntries.map(
        async (stockCountEntry) => {
          if (stockCountEntry.id) {
            ...
            await manager.update(
              StockCountEntry,
              { clientId: user.clientId, id: stockCountEntry.id },
              updateData,
            );
          } else {
            stockCountEntry.id = uuidv4();
  ...
  async getPartialStockCountEntries(
    ...
    const query = this.repository.connection
      .createQueryBuilder(StockCountEntry, 'SE')
      ...
      .where('SE.clientId = :clientId AND SE.stockCountId = :stockCountId', {
        clientId: user.clientId,
        stockCountId,
      });

    if (status == 'counted') {
      query.andWhere('SE.status = :counted', {
        counted: StockCountEntryStatus.COUNTED,
      });
    } else if (status == 'uncounted') {
      query.andWhere('SE.status = :uncounted', {
        uncounted: StockCountEntryStatus.UNCOUNTED,
      });
  ...
  async getStockCountEntriesByStatus(
    ...
    if (countType === StockCountStatus.PARTIAL) {
      return await this.getPartialStockCountEntries(
        user,
        stockCountId,
        status,
        limit,
      ...
      const stockCEntryQuery = this.repository.connection
        .createQueryBuilder(StockCountEntry, 'SCE')
        .innerJoin('SCE.product', 'P')
        .innerJoin('SCE.productVariant', 'PV')
        .select([
        ...
        .where(
          'SCE.clientId = :clientId AND SCE.stockCountId = :stockCountId',
          { clientId: user.clientId, stockCountId },
        );

      const pVariantQuery = this.repository.connection
        .createQueryBuilder(ProductVariant, 'PV')
        .innerJoin('PV.product', 'P')
        .leftJoin(
          'PV.productLocations',
      ...
      if (status == 'uncounted') {
        stockCEntryQuery.andWhere(
          `SCE.status = "${StockCountEntryStatus.UNCOUNTED}"`,
        );
      }
      ...
      if (status != 'uncounted') {
        stockCountEntries = await stockCEntryQuery
          .offset(offset)
          .limit(limit)
          .orderBy('P.name')
          .getRawMany();
      ...
      if (entryLen) {
        exceptProductVariantIds = stockCountEntries.map(
          (entry) => entry.productVariantId,
        );
      }

      if (status == 'all' || status == 'uncounted') {
        const countPVEntries = await this.repository.connection
          .createQueryBuilder(StockCountEntry, 'SCE')
          .select(['SCE.id id', 'SCE.productVariantId productVariantId'])
          .where(
            'SCE.clientId = :clientId AND SCE.stockCountId = :stockCountId',
            { clientId: user.clientId, stockCountId },
          )
        ...
        if (exceptProductVariantIds.length)
          pVariantQuery.andWhere('PV.id NOT IN(:productVariantIds)', {
            productVariantIds: exceptProductVariantIds,
          });
        ...
        if (limit - entryLen > 0) {
          ...
          uncountedEntries = await pVariantQuery
            .offset(variantOffset)
            .limit(limit - entryLen)
            .getRawMany();
        }
      ...
      return this.responses(data, total, limit, offset);
    }
  }
  ...
  async discardStockCount(user: User, id: string) {
    await getManager().transaction(async (manager) => {
      ...
      await manager.delete(StockCountEntry, {
        clientId: user.clientId,
        stockCountId: id,
      });
    });
