...
export default class StockAdjustmentRepository extends BaseRepository<StockAdjustment> {
    ...
    findMany(filter: Filter): Promise<[StockAdjustment[], number]> {
        const query = this.connection.createQueryBuilder(StockAdjustment, "SA")
            .innerJoinAndSelect("SA.location", "Location")
            .innerJoinAndSelect("SA.user", "User")
            .leftJoinAndSelect("SA.approver", "Approver");

        if (!!filter.similar) {
            query.andWhere("(SA.description LIKE :key OR SA.subject LIKE :key OR User.fullName LIKE :key)", { key: filter.similar });
        }

        if (filter.rangFilter) {
            if (Utils.isJsonString(filter.rangFilter)) {
                const rangFilter = JSON.parse(filter.rangFilter);
                query.andWhere(`DATE_FORMAT(SA.createdAt, '%Y-%m-%d') >= :startDate AND DATE_FORMAT(SA.createdAt, '%Y-%m-%d') <= :endDate`, {startDate: rangFilter["value"][0], endDate: rangFilter["value"][1]});
            }
        }
    ...
    findManyOfApproveList(filter: Filter): Promise<[StockAdjustment[], number]> {
        ...
        return this.findManyQueryBuilder(StockAdjustment, relationShipCollection, filter, [conditionString, conditionObject]).getManyAndCount();
    }

    detail(id: string): Promise<StockAdjustment> {
        return this.connection.createQueryBuilder(StockAdjustment, "SA")
