...
export default class StockTransferRepository extends BaseRepository<StockTransfer> {
    ...
    async findMany(filter: Filter): Promise<[StockTransfer[], number]> {
        ...
        return await this.findManyQueryBuilder(StockTransfer, relationShipCollection, filter, [conditionString, conditionObject]).getManyAndCount();
    }

    async findManyOfReceiveList(filter: Filter): Promise<[StockTransfer[], number]> {
        ...
        var conditionObject = {step: [TransferStepEnums.RECEIVED, TransferStepEnums.PROCESS]};
    ...
    async detail(stockTransferId: string, languageId: string): Promise<StockTransfer> {

        const entities: string[] = ["StocktransferEntry", "Supplier", "User", "Location"];
            ...
            [
                {
                        ...
                        relation: [
                            {
                                ...
                                relation: {
                                    ...
                                    condition: ['productDescriptions.languageId = :languageId', {languageId}]
                                }
                            },
                            {
                    ...
                    condition: [​`${entities[0]}.status = :purchaseOrderEntryStatus`, {purchaseOrderEntryStatus: RecordStatus.Active}]
                }
            ],
            {},
            [`${this.entityName}.id = :stockTransferId`, {stockTransferId}]).getOne();
    }

    async findManyV2(filter: Filter) {
        const query = this.connection.createQueryBuilder(StockTransfer, "S")
        ...
        if (filter.similar != null && filter.similar != "undefined") {
            query.andWhere(`(S.name LIKE :search OR S.description LIKE :search OR User.fullName LIKE :search OR FL.name LIKE :search)`, {search: `%${filter.similar}%`});
        }

        if (filter.condition) {
            if (filter.condition["fromLocationId"]) query.andWhere("S.fromLocationId = :fromLocationId", {fromLocationId: filter.condition["fromLocationId"]});
            if (filter.condition["toLocationId"]) query.andWhere("S.toLocationId = :toLocationId", {toLocationId: filter.condition["toLocationId"]}); 
        }

        if (filter.rangFilter) {
            if (Utils.isJsonString(filter.rangFilter)) {
                const rangFilter = JSON.parse(filter.rangFilter);
                query.andWhere(`DATE_FORMAT(S.createdAt, '%Y-%m-%d') >= :startDate AND DATE_FORMAT(S.createdAt, '%Y-%m-%d') <= :endDate`, {startDate: rangFilter["value"][0], endDate: rangFilter["value"][1]});
            }
        }
    ...
    async detailV2(id: string, clientId: string): Promise<StockTransfer> {
        return await this.connection.createQueryBuilder(StockTransfer, "ST")
