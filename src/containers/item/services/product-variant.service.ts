...
export default class ProductVariantService extends BaseService<ProductVariant> {
    ...
    async getDetailWithLocation(clientId: string, id: string, locationId: string) {
        const query = await this.repository.connection.createQueryBuilder(ProductVariant, "PV")
        ...
        .where("PV.clientId = :clientId AND PV.id = :id AND PL.locationId = :locationId", {clientId, id, locationId})
        .getRawOne();
   	...
   	async archive(id: string) {
        ...
        const result = await this.repository.findOneByFieldName({id}, ["quantity"]);
    ...
    async checkDeleteAvailability(id: string): Promise<boolean> {
        const result = await this.repository.findOneByFieldName({id}, ["quantity"]);
    ...
    async checkIsAvailableForArchiveAttributeValue(productAttributeValueId: string) {
        const result = await this.repository.findOneByFieldName({productAttributeValueId: Like(`%${productAttributeValueId}%`), quantity: MoreThan(0)}, ["quantity"]);
    ...
    async checkIsAvailableForArchiveAttribute(productAttributeId: string) {
       const result = await this.productAttributeValueRepository.findMany({condition: {productAttributeId}, select: ["id"]});

        for (var i = 0; i < result[0].length; i++) {
            const attributevalue = await this.repository.findOneByFieldName({productAttributeValueId: Like(`%${result[0][i].id}%`), quantity: MoreThan(0)}, ["quantity"]);
    ...
    async getProductsReport(filter: Filter, viewStock: string, conditionId: string) {
        const baseQuery = () => {
            ...
            if (filter.similar && filter.similar != "0") {
                query.andWhere(`(P.name LIKE '%${filter.similar}%' OR P.namekm LIKE '%${filter.similar}%' OR PV.name LIKE '%${filter.similar}%' OR PV.barcode LIKE '%${filter.similar}%')`);
            }
        ...
        const summaryQuery = baseQuery().innerJoin("PV.product", "P", "P.status = 1");
        ...
        const result = this.responses(await resultQuery.getMany(), await resultQuery.getCount(), filter.limit, filter.offset);
        ...
        const currentStockValueByCost = _.sumBy(summary, value => value.cost * value.quantity);
        const currentStockValueByPrice = _.sumBy(summary, value => value.price * value.quantity);
        const expectedProfit = Math.max(currentStockValueByPrice - currentStockValueByCost, 0);
    ...
    async getInventoryDashboard(clientId: string) {
        const stockByConditions = await this.repository.connection.createQueryBuilder(Product, "P")
        .innerJoin("P.productVariants", "PV", "PV.clientId = :clientId", {clientId})
    ...
    async getPopularCategories(clientId: string, limit: number, locationId: string) {
        return await this.repository.connection.query(`
            ...
            AND date_format(t.createdAt, '%Y-%m') >= '${moment().subtract(90, "days").format("YYYY-MM")}'
            ORDER BY total DESC
            LIMIT ${limit}
        `);
    }

    async getPopularProducts(clientId: string, limit: number, popularBy: string, locationId: string) {
        const todayPurchases = await this.repository.connection.query(`
            ...
            having clientId = '${clientId}'
            and step = ${TransactionStep.PAID}
            and date_format(createdAt, '%Y-%m') >= '${moment().subtract(30, "days").format("YYYY-MM")}'
    ...
    async getTodayPurchases(clientId: string, locationId: string) {
        return await this.repository.connection.query(`
            ...
            WHERE po.clientId = '${clientId}'
                ...
                AND date_format(po.createdAt, '%Y-%m-%d') = '${moment().format("YYYY-MM-DD")}'
                AND pe.status = 1
    ...
    async exportProduct(locationId: string, viewStock: string, clientId: string) {
        ...
        query = this.repository.connection.createQueryBuilder("ProductVariant", "PV")
        ...
        if (locationId) {
            query.leftJoin("PV.productLocations", "PL", "PL.locationId = :locationId", {locationId});

            selectColumns.push("SUM(IFNULL(PL.quantity, 0)) AS quantity");
            selectColumns.push("TRUNCATE(PV.cost * SUM(IFNULL(PL.quantity, 0)), 2) AS totalCost");
            selectColumns.push("TRUNCATE(PV.price * SUM(IFNULL(PL.quantity, 0)), 2) AS totalPrice");
        } else {
