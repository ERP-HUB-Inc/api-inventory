...
export default class PromotionService extends BaseService {
  ...
  async getPromotionById(user: User, id: string) {
    const promotion: Promotion = await getConnection()
      .createQueryBuilder(Promotion, 'P')
      .where('P.id = :id AND P.clientId = :clientId', {
        id,
    ...
    if (promotion.type == Promotion.BASIC) {
      const productDiscounts: ProductDiscount[] = await getConnection()
        ...
        .where('PD.promotionId = :promotionId AND PD.clientId = :clientId', {
      ...
      const promotionCriteria: PromotionCriteria = await getConnection()
        ...
        .where('PC.clientId = :clientId AND PC.promotionId = :promotionId', {
          clientId: user.clientId,
          promotionId: promotion.id,
        })
        .getOne();
  ...
  async createNewPromotion(data: Promotion) {
    await getManager().transaction(async (manager) => {
      ...
      if (data.type == Promotion.BASIC) {
        ...
        const whenBuyProducts: PromotionCriteriaProduct[] = [
          ...promotionCriteria.whenBuyProducts,
        ];
        const thenGetProducts: PromotionCriteriaProduct[] = [
          ...promotionCriteria.thenGetProducts,
        ];
        ...
        promotionCriteria = await manager.save(
          PromotionCriteria,
          promotionCriteria,
        );

        if (
          promotionCriteria.whenTarget == PromotionCriteria.WHEN_TARGET.SPECIFIC
        ) {
          ...
          if (productCriterias.length)
            await manager.insert(PromotionCriteriaProduct, productCriterias);
        }

        if (
          promotionCriteria.thenTarget == PromotionCriteria.THEN_TARGET.SPECIFIC
        ) {
          const productCriterias: PromotionCriteriaProduct[] = [];
  ...
  async updatePromotion(id: string, data: Promotion) {
    await getManager().transaction(async (manager) => {
      ...
      if (data.type == Promotion.BASIC) {
        if (productsDiscount && productsDiscount.length) {
          const promises = productsDiscount.map(async (entry) => {
            if (entry.id) {
              ...
              await manager.update(
                ProductDiscount,
                { id: entry.id, clientId: data.clientId },
                updateEntry,
              );
            } else {
        ...
        await manager.update(
          PromotionCriteria,
          { clientId, id: promotionCriteria.id },
          promotionCriteria,
        );
          ...
          const promisesBuy = whenBuyProducts.map(async (whenBuyProduct) => {
            if (whenBuyProduct.id) {
              if (whenBuyProduct['status'] == RecordStatus.Archive) {
                await manager.delete(PromotionCriteriaProduct, {
                  clientId,
                  id: whenBuyProduct.id,
                });
              } else {
                await manager.update(
                  PromotionCriteriaProduct,
                  { clientId, id: whenBuyProduct.id },
                  whenBuyProduct,
                );
              }
            } else {
          ...
          const promisesGet = thenGetProducts.map(async (thenGetProduct) => {
            if (thenGetProduct.id) {
              if (thenGetProduct['status'] == RecordStatus.Archive) {
                await manager.delete(PromotionCriteriaProduct, {
                  clientId,
                  id: thenGetProduct.id,
                });
              } else {
                await manager.update(
                  PromotionCriteriaProduct,
                  { clientId, id: thenGetProduct.id },
                  thenGetProduct,
                );
              }
            } else {
  ...
  async deletePromotion(data: string, clientId: string) {
    await getManager().transaction(async (manager) => {
      ...
      await manager
        .createQueryBuilder(Promotion, 'P')
        .where(`id IN(:ids) AND clientId = "${clientId}"`, { ids })
        .delete()
        .execute();
      await manager
        .createQueryBuilder(ProductDiscount, 'PD')
        .where(`promotionId IN(:ids) AND clientId = "${clientId}"`, { ids })
        .delete()
        .execute();
      await manager
        .createQueryBuilder(PromotionCriteria, 'PC')
