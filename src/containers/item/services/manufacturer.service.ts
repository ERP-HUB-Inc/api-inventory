...
export default class ManufacturerService {
  ...
  async findMany(filter: Filter) {
    ...
    return this.responses(
      plainToInstance(ManufacturerResponseDto, results[0], {
  ...
  async findOne(
    ...
    return this.response(
      plainToInstance(
        ManufacturerResponseDto,
        await this.repository.findOneByFieldName(
          condition,
          select,
          relations,
          joinOptions,
  ...
  async create(data: ManufacturerCreateDto, columnCheckExist?: Object) {
    console.log(
      plainToInstance(Manufacturer, data, { excludeExtraneousValues: true }),
    );
    return this.response(
      plainToInstance(
        ManufacturerResponseDto,
        await this.repository.create(
          plainToInstance(Manufacturer, data, {
            excludeExtraneousValues: true,
          }),
          columnCheckExist,
        ),
