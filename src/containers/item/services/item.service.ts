...
import { fromItemDtoToItem, fromItemUpdateDtoToItemDto, fromVariantDtoToVariant } from "../mappers";
import { 
    VariantDto,
    ProductUpdateDto,
    VariantRequestDto,
...
import { ProductResponseDto } from "@inventory/dto/ProductResponseDto";
...
export default class ItemService {
    ...
    constructor() {
        ...
        this.productVariantRepository = getCustomRepository(ProductVariantRepository);
        this.clientRepository = getCustomRepository(ClientRepository);
        this.productLocationRepository = getCustomRepository(ProductLocationRepository);
    ...
    responses(datas: Product[], total: number = datas.length, limit: number = 0, offset: number = 0) {
        return {
            data: datas,
            pagination: {
                total,
    ...
    responsesT<D>(datas: D[], total: number = datas.length, limit: number = 0, offset: number = 0) {
        return {
            data: datas,
            pagination: {
                total,
        ...
        await getManager().transaction(async manager => {
            ...
            if (isSavingNewRecord) {
                ...
                if (_.isEmpty(product.categoryId)) {
                    let productType = await this.CategoryRepository.connection.createQueryBuilder<ProductType>("Category", "PT")
                    .select(["id"])
                    .where("isDefault = 1")
                    .getRawOne();
                    if (productType == null) {
                ...
                if (_.isEmpty(product.brandId)) {
                    let brand: Brand = await this.brandRepository.connection.createQueryBuilder<Brand>("Brand", "B")
                    .select(["id"])
                    .where("isDefault = 1")
                    .getRawOne();
                    if (brand == null) {
                ...
                if (_.isEmpty(product.stockUnitId)) {
                    unit = await this.unitRepository.connection.createQueryBuilder(Unit, "U")
                    .select(["id"])
                    .where("isDefault = 1")
                    .getRawOne();
                    if (unit == null) {
                    ...
                    unit = await this.repository.connection.createQueryBuilder(Unit, "U")
                    .select("name")
                ...
                product = await this.repository.findOneByFieldName({ id: condition["id"] });
                product = Object.assign(product, data);
            ...
            if (!data.id) {
                data.productOption = productVariants && productVariants.length > 0 ? ProductionOptionEnums.VARIANT : ProductionOptionEnums.STANDARD;
            }

            data.minPrice = data.price;

            const resultProduct = await manager.save(Product, fromItemDtoToItem(data));

            id = resultProduct.id;

            if (resultProduct.productOption != ProductionOptionEnums.VARIANT) {
                const productVariant = {
                    id: isUpdateProduct ? condition["id"] : await this.repository.generateUUId(),
                    productId: resultProduct.id,
                    name: "",
                    image: "",
                    barcode: data.isAutoGenerateBarcode == TypeOfProductGenerateSKU.AUTO ? await this.clientRepository.generateProductCodeSequenceStart() : data.barcode,
                ...
                if (data.isAutoGenerateBarcode == TypeOfProductGenerateSKU.AUTO && !isUpdateProduct) {
                    productVariant["barcode"] = await this.clientRepository.generateProductCodeSequenceStart();
                }
            ...
            if (Array.isArray(productVariants)) {
                const promiseSaveVariants = productVariants.map(async (variantDto: any) => {
                    ...
                    if (variantDto) {
                        ...
                        productVariant.barcode = variantDto.isAutoGenerateBarcode == TypeOfProductGenerateSKU.AUTO ? await this.clientRepository.generateProductCodeSequenceStart() : variantDto.barcode;
            ...
            const foundProductVariantMinPrice = _.minBy(productVariants, productVariant => productVariant["price"]);
            if (foundProductVariantMinPrice) await manager.update(Product, {id: resultProduct.id}, {minPrice: foundProductVariantMinPrice["price"]})

            //Sync product to MKP Database
            const enableSync = false
            if (enableSync) {
                let ec_product = await getConnection("MKP").createQueryBuilder("Product", "P")
                .where("P.sync_product_id = :productId", { productId: resultProduct.id })
                .getOne();
                ...
                if (ec_product) {
                    await getConnection("MKP").createQueryBuilder()
                    .update("Product")
                    .set(ec_product_data)
                    .where("sync_product_id = :productId", { productId: resultProduct.id })
                    .execute();
                } else {
                    await getConnection("MKP").createQueryBuilder()
                    .insert()
    ...
    async getProductById(productId: string, productOption: number, isIncludeLocation: boolean) {
        const query = this.repository.connection.createQueryBuilder(Product, "P")
        ...
        if (productOption == ProductionOptionEnums.VARIANT) {
            result.productAttributes = await this.repository.connection.createQueryBuilder<ProductAttribute>("ProductAttribute", "PA")
    ...
    async getVariantsByItemId(dto: VariantRequestDto): Promise<Record<string, any>> {
        const results = await this.itemRepository.findVariantsByItemId(dto);
        return this.responsesT<VariantDto>(plainToInstance(VariantDto, results[0], { excludeExtraneousValues: true }), results[1], dto.limit, dto.offset);
    }
    ...
    async getFormData(clientId: string) {
        const conditions: Condition[] = await this.repository.connection.createQueryBuilder(Condition, "C")
        .select(["id", "name"])
    ...
    async findDetailStockByProductId(clientId: string, productId: string) {
        let productVariants = await this.repository.connection.createQueryBuilder("ProductVariant", "PV")
        ...
        .where("PV.clientId = :clientId AND PV.productId = :productId", {clientId, productId})
        .getMany();

        productVariants = productVariants.map(productVariant => {
            productVariant["productLocations"] = productVariant["productLocations"].map(productLocation => ({location: productLocation.location.name, quantity: productLocation.quantity, lastModified: productLocation.updatedAt}));
    ...
    async getPurchaseByProduct(clientId: string, productId: string, startDate: string, endDate: string, offset: number, limit: number) {
        const query = this.repository.connection.createQueryBuilder("PurchaseOrder", "PO")
        .innerJoin("PO.location", "L")
        .innerJoin("PO.supplier", "S")
        .innerJoin("PO.purchaseOrderEntries", "PE", "PE.status = 1 AND PE.clientId = :clientId")
        ...
        query.where("PO.clientId = :clientId AND P.id = :productId", {clientId, productId});

        if (offset) query.offset(offset);
        if (limit) query.limit(limit);
        
        if (startDate && endDate) query.andWhere("DATE_FORMAT(PO.createdAt, '%Y-%m-%d') >= :startDate AND DATE_FORMAT(PO.createdAt, '%Y-%m-%d') <= :endDate", {startDate, endDate});
        
        query.andWhere("PO.status = 1 AND PO.step = :step", {step: PurchaseStepEnums.RECEIVED})
    ...
    async findDetailForSplit(productVariantId: string, locationId: string) {
        const result: ProductVariant = await this.repository.connection.createQueryBuilder<ProductVariant>("ProductVariant", "PV")
        .innerJoin("PV.product", "P")
        .innerJoin("P.unitOfMeasurement", "U")
        .leftJoin("PV.productLocations", "PL", "PL.locationId = :locationId", {locationId})
        ...
        result.productPackages = await this.repository.connection.createQueryBuilder<ProductPackage>("ProductPackage", "PP")
        .innerJoin("PP.rawProduct", "PV")
        .innerJoin("PV.product", "P")
        .innerJoin("P.unitOfMeasurement", "U")
        .innerJoin("PV.productLocations", "PL", "PL.locationId = :locationId", {locationId})
        .select([
            "PP.id AS id",
            "PV.id AS productVariantId",
            "P.image AS image",
    ...
    async modifyCost(productVariantId: string, cost: number, clientId: string) {
        ...
        .where("clientId = :clientId AND id = :productVariantId", {clientId, productVariantId})
        .execute();
    }

    async findManyForDropDown(filter: Filter, isSearchingBarcode: boolean) {
        const results = await this.repository.findManyForDropDown(filter, isSearchingBarcode);
        return this.responses(results[0], results[1], filter.limit, filter.offset);
    }

    async searchProducts(filter: Filter, isSearchingBarcode: boolean) {
        const results = await this.itemRepository.searchProducts(filter, isSearchingBarcode);
        return this.responses(results[0], results[1], filter.limit, filter.offset);
    }
    ...
    async editProductCost(user: User, productVariantId: string, newCost: number) {
        await getManager().transaction(async manager => {
            ...
            .where("PV.clientId = :clientId AND PV.id = :productVariantId", {clientId: user.clientId, productVariantId})
            .getRawOne();

            await manager.update(ProductVariant, {clientId: user.clientId, id: productVariantId}, {cost: newCost});

            const productLocation: ProductLocation = await this.repository.connection.createQueryBuilder(ProductLocation, "PL")
            .select("quantity")
            .where("PL.clientId = :clientId AND PL.locationId = :locationId AND PL.productVariantId = :productVariantId", {clientId: user.clientId, locationId: user.locationId, productVariantId})
            .getRawOne();
            ...
            if (productLocation) {
                ...
                await this.movementLogRepsitory.log(movementLog, StockEnums.EDIT_COST, manager);
            }
        });
    }

    async splitProduct(productVariantId: string, quantityToSplit: number, clientId: string, locationId: string, outputProducts: Product[]) {
        await getManager().transaction(async manager => {
            const mainProduct = await manager.connection.createQueryBuilder("ProductVariant", "PV")
            .innerJoin("PV.productLocations", "PL")
            .select([
                "PV.quantity AS allQuantity",
                "PL.quantity AS currentLocationQuantity"
            ])
            .where("PL.clientId = :clientId AND productVariantId = :productVariantId AND PL.locationId = :locationId", {clientId, productVariantId, locationId})
            .getRawOne();

            if (mainProduct && mainProduct["currentLocationQuantity"] >= quantityToSplit) {
                await manager.update(ProductLocation, {clientId, productVariantId, locationId}, {quantity: mainProduct["currentLocationQuantity"] - quantityToSplit});
                await manager.update(ProductVariant, {clientId, id: productVariantId}, {quantity: mainProduct["allQuantity"] - quantityToSplit});

                const promises = outputProducts.map(async outputProduct => {
                    const product = await manager.connection.createQueryBuilder("ProductVariant", "PV")
                    .leftJoin("PV.productLocations", "PL", "PL.locationId = :locationId", {locationId})
                    .select([
                        "PV.quantity AS allQuantity",
                        "cost",
                    ...
                    .where("PV.clientId = :clientId AND PV.id = :productVariantId", {clientId, productVariantId: outputProduct["productVariantId"], locationId})
                    .getRawOne();

                    if (product["locationId"] && product["currentLocationQuantity"]) {
                        await manager.update(ProductLocation, {clientId, locationId, productVariantId: outputProduct["productVariantId"]}, {quantity: product["currentLocationQuantity"] + outputProduct["quantity"]});
                    } else {
                        ...
                        productLocation.productVariantId = outputProduct["productVariantId"];
                    ...
                    const newCost: number = productVariant.getAvgCost(outputProduct["quantity"], outputProduct["cost"]);
                    await manager.update(ProductVariant, {clientId, id: outputProduct["productVariantId"]}, {quantity: product["allQuantity"] + outputProduct["quantity"], cost: newCost});

                    if (outputProduct["id"]) {
                        if (outputProduct["status"] == RecordStatus.Archive) 
                            await manager.delete(ProductPackage, {clientId, id: outputProduct["id"]});
                    } else {
        ...
        getManager().transaction(async manager => {
            ...
            for (let i = 1; i < lenght; i++) {
                ...
                if (value["A"]) {
                    ...
                    //Check exist product
                    const existProductVariant = await this.productVariantRepository.connection.createQueryBuilder<ProductVariant>("ProductVariant", "PV")
                        .select(["id"])
                        .where("clientId = :clientId AND barcode = :barcode", {clientId, barcode})
                        .getRawOne();
                    ...
                    const existProInList = products.find((product, index) => product["B"] == barcode && index > 0 && index < i);
                    if (existProInList) {
                        duplicateProducts.push({name: existProInList["A"], barcode: existProInList["B"]});
                    }
                    ...
                    let category: ProductType = await this.CategoryRepository.connection.createQueryBuilder<ProductType>("ProductType", "Category")
                        .select(["id"])
                        .where("clientId = :clientId AND (name = :name OR namekm = :name)", {clientId, name: categoryName})
                        .getRawOne();
                    ...
                    if (category) {
                        ...
                        const existInCategoryList = categories.find((value, index) => value.name == categoryName && index < i);
                    ...
                    let unit: Unit = await this.unitRepository.connection.createQueryBuilder<Unit>("Unit", "U")
                        .select(["id"])
                        .where("clientId = :clientId AND name = :unitName", { clientId, unitName })
                        .getRawOne();
                    ...
                    if (unit) {
                        ...
                        unit.multiple = 1;
                        unit.label = unitName;
                        const existUnitInList = units.find((value, index) => value.name == unitName && index < i);
                        if (existUnitInList) {
                    ...
                    productVariant.id = await this.productVariantRepository.generateUUId();
                    ...
                    if (openingStock) {
                        ...
                        let location: Location = await this.locationRepository.connection.createQueryBuilder<Location>("Location", "Location")
                            .select(["id"])
                        ...
                        if (!location) {
                            ...
                            location.isSystem = 1;
                            const resultLocation: Location = await manager.save(Location, location);
