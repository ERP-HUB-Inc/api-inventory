...
export default class LoyaltyProgramRepository extends BaseRepository<LoyaltyProgram>  {
    async getLoyaltyProgramById(user: User, id: string) {
        const query = getConnection().createQueryBuilder(LoyaltyProgram, "LP")
        ...
        .where("LP.clientId = :clientId AND LP.id = :id", {clientId: user.clientId, id})
        .orderBy("Reward.cost");
    ...
    getLoyaltyProgram(clientId: string): Promise<LoyaltyProgram> {
        return getConnection().createQueryBuilder(LoyaltyProgram, "LP")
        .select([
            "id",
            "pointPerAmount",
            "pointPerOrder",
    ...
    async updateById(user: User, data: LoyaltyProgram, id: string) {
        await getManager().transaction(async manager => {
            await manager.update(LoyaltyProgram, {clientId: user.clientId, id}, data);
            const deleteIds = [];
            data.rewards = data.rewards.map(reward => {
            ...
            if (deleteIds.length) {
                await manager.createQueryBuilder(Reward, "PD").where("id IN(:ids) AND clientId = :clientId", {ids: deleteIds, clientId: user.clientId}).delete().execute();
            }
        });
    }
    ...
    async delete(user: User, loyaltyProgramId: string) {
        await getManager().transaction(async manager => {
            await manager.createQueryBuilder(LoyaltyProgram, "P").where(`id = :loyaltyProgramId AND clientId = "${user.clientId}"`, {loyaltyProgramId}).delete().execute();
            await manager.createQueryBuilder(Reward, "PD").where(`loyaltyProgramId = :loyaltyProgramId AND clientId = "${user.clientId}"`, {loyaltyProgramId}).delete().execute();
        });
