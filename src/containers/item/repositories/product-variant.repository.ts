...
export default class ProductVariantRepository extends BaseRepository {
  ...
  async findMany(filter: Filter): Promise {
    ...
    if (!filter.locationId) {
      const locationRelation: QueryBuilderRelationship = {
        ...
        condition: [
          `${entities[2]}.locationId = :locationId`,
  ...
  async findDetailByLocation(id: string, locationId: string): Promise {
    const entities: string[] = ['Product', 'ProductLocation'];
